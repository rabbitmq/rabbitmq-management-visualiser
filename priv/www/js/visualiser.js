...
dispatcher_add(function(sammy) {
    sammy.get('#/visualiser', function() {
        render({}, 'visualiser', '#/visualiser');
        visualiser = new Visualiser();
        visualiser.visualisationStart();
    });
});
...
function update() {
    if($('#visualiser').length == 0) {
        visualiser.disableRendering();
        return;
    }
    clear_update_timeout();
    if (undefined === selectedVhost) {
        with_req("GET", "/all", "", updateReady);
    } else {
        with_req("GET", "/all/" + encodeURIComponent(selectedVhost), "", updateReady);
    }
}
...
function set_update_timeout(){
    if (timer_interval != null) {
        update_timeout = setTimeout(update, timer_interval);
    }
}

function clear_update_timeout() {
    if(update_timeout){
...
function set_details_update_timeout() {
    if (timer_interval != null) {
        details_timeout = setTimeout(repeatGetDetails, timer_interval);
    }
}

function clear_details_timeout() {
    if(details_timeout){
...
this.visualisationStart = function() {
...
this.showExchanges = function() {
    show("hidden_exchanges", model, 'exchange');
}

this.showQueues = function() {
...
this.toggleRendering = function(hiddenElemId, showButtonElemId, type) {
    var hidden, i, e;
