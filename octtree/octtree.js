...
octtree.findInRadius = function(tree, pos, radius, limit) {
	...
	while (0 < worklist.length && (undefined == limit || limit > acc.length)) {
		...
		if (x_m_r < tree.xMid) {
			if (y_m_r < tree.yMid) {
				if (z_m_r < tree.zMid) {
					worklist.push(tree[octtree.bot_sw]);
				}
				if (tree.zMid <= z_p_r) {
					worklist.push(tree[octtree.bot_nw]);
				}
			}
			if (tree.yMid <= y_p_r) {
				if (z_m_r < tree.zMid) {
					worklist.push(tree[octtree.top_sw]);
				}
				if (tree.zMid <= z_p_r) {
					worklist.push(tree[octtree.top_nw]);
				}
			}
		}
		if (tree.xMid <= x_p_r) {
			if (y_m_r < tree.yMid) {
				if (z_m_r < tree.zMid) {
					worklist.push(tree[octtree.bot_se]);
				}
				if (tree.zMid <= z_p_r) {
					worklist.push(tree[octtree.bot_ne]);
				}
			}
			if (tree.yMid <= y_p_r) {
				if (z_m_r < tree.zMid) {
					worklist.push(tree[octtree.top_se]);
				}
				if (tree.zMid <= z_p_r) {
					worklist.push(tree[octtree.top_ne]);
				}
			}
		}
	}
